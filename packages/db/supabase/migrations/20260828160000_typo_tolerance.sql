-- Tippfehler je Titel messen, nicht ueber die Verkettung.
--
-- Der Filter verglich die Eingabe mit `film_search_text(...)`, also mit
-- allen drei Titeln aneinandergehaengt. Ein laengerer Text teilt anteilig
-- weniger Trigramme mit einer kurzen Eingabe:
--
--   similarity('Solaris Солярис Solaris', 'solars') = 0.278
--   similarity('Solaris',                 'solars') = 0.500
--
-- Die Schwelle liegt bei 0.3. "Solars" fand Solaris also nicht: ein
-- fehlender Buchstabe genuegte. Der Testfall dazu war trotzdem gruen:
-- das eingebettete Postgres lief in der C-Locale, in der pg_trgm
-- nichtlateinische Zeichen aus den Trigrammen wirft, und die Verkettung
-- blieb dadurch zufaellig kurz genug. Erst der CI-Rechner mit der Locale
-- des Projekts hat es gezeigt.
--
-- Die Bewertung nahm ohnehin schon das Maximum je Titel. Nur der Filter
-- tat es nicht, und was der Filter aussortiert, kann keine Bewertung
-- mehr retten.
--
-- Ausser dem Filter ist an der Funktion nichts geaendert.

-- Wie in 20260826090400: pg_trgm liegt auf Supabase im Schema
-- extensions, hier im Pfad statt am Namen.
set search_path = public, extensions;

create index if not exists films_title_de_trgm on public.films
  using gin (title_de gin_trgm_ops);
create index if not exists films_title_original_trgm on public.films
  using gin (title_original gin_trgm_ops);
create index if not exists films_title_en_trgm on public.films
...
comment on function public.search_films(text, integer) is
  ...
  'rank identically on web, iOS and Android. Similarity is measured per '
  'title: across the concatenation it thins out until one missing letter '
  'costs the hit.';
